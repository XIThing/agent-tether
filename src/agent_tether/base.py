...
class BridgeConfig:
    ...
    # Default adapter label shown in messages
    default_adapter: str | None = None

    # Error debounce (seconds, 0 = no debounce)
    error_debounce_seconds: int = 0


# Callback type for looking up a session's directory by session_id.
# Returns the directory path or None.
GetSessionDirectory = Callable[[str], str | None]

# Callback type for getting session info (directory, adapter, state, etc.)
# Returns a dict with session fields or None.
GetSessionInfo = Callable[[str], dict | None]
...
class BridgeInterface(ABC):
    ...
    def _check_dir_auto_approve(self, session_id: str, now: float) -> str | None:
        ...
        if not self._allow_dir_until:
            return None
        if not self._get_session_directory:
            return None
        directory = self._get_session_directory(session_id)
        if not directory:
            return None
        sess_dir = os.path.normpath(directory)
    ...
    def _should_send_error_status(self, session_id: str) -> bool:
        ...
        """
        debounce_s = max(0, self._config.error_debounce_seconds)
        if debounce_s == 0:
            return True
